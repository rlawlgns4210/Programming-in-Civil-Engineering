...
def draw_side_bridge_and_forces(forces, stresses, height, remaining_length, num_panels, save_path=None):
    a = np.arctan(2)
    ...
    for i in range(num_panels):
        ...
        if i == 0:
            plt.annotate('', xy=(left_x - remaining_length - 3, base_y), xytext=(left_x - remaining_length - 3, apex_y), arrowprops=dict(arrowstyle='<->'))
            plt.text(left_x - remaining_length - 3.5, base_y + height / 2, f'{height:.1f} m', ha='center', va='center', rotation='vertical')
            arc_radius = 3
    ...
    plt.annotate('', xy=(0, -2), xytext=(-remaining_length, -2), arrowprops=dict(arrowstyle='<->'))
    plt.text(-remaining_length / 2, -2.5, f'{remaining_length:.1f} m', ha='center', va='center')

    apex_x_points, apex_y_points = zip(*apex_points)
    plt.plot(apex_x_points, apex_y_points, 'b-')

    plt.plot([0, -remaining_length], [0, 0], 'r-')
    plt.plot([-remaining_length, 5], [0, 10], 'r-')
    ...
    plt.title('측교')
    plt.xlim(-remaining_length - 5, total_length + 5)
    plt.ylim(-5, height + 5)
    plt.gca().set_xticks([])
    plt.gca().set_yticks([])
